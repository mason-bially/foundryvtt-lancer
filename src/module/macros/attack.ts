...
type AttackResult = {
  ...
  token: { name: string; img: string } | null;
  total: string;
  hit: boolean;
  crit: boolean;
};
...
async function rollAttackMacro(
  ...
  const attacks = await checkTargets(atkRolls, isSmart);
  ...
  const has_normal_hit =
    (attacks.length === 0 && !!attacks.find(attack => (attack.roll.total ?? 0) < 20)) ||
    !!attacks.find(hit => hit.hit && !hit.crit);
  const has_crit_hit =
    (attacks.length === 0 && !!attacks.find(attack => (attack.roll.total ?? 0) >= 20)) || !!attacks.find(attacks => attacks.crit);
